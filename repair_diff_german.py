...
def get_path_dict():
    saved_model_path = "models/finetuned_models_protected_attributes/german/"
    path_ls = os.listdir(saved_model_path)
    path_dict = {}
    path_dict['a'] = [saved_model_path+p for p in path_ls if "a_german" in p]
    path_dict['a'].sort()
    path_dict['g'] = [saved_model_path + p for p in path_ls if "g_german" in p]
    path_dict['g'].sort()
    print(path_dict)
    return path_dict
...
def similar_set(X, num_attribs, protected_attribs, constraint, name):
    ...
    file_name = f"similar_set/german_{args.attr}_{name}_similar_set"
...
def get_penalty_awarded(top_n, layer_num, total_num, german_critical, protected_critical_ls):
    neurons = []

    for i in range(layer_num):
        german_layer_critical = german_critical[i].flatten()
        i_critical = my_filter(german_layer_critical, total_num)
...
def retrain(k, ps, neurons, para_res):
    ...
    new_model.load_weights(args.german_path, by_name=True)
    ...
    losses_weights = {'layer6': 5000.0, tf_name: 0.1, diff_name: 0.4}

    new_model.compile(loss=losses, loss_weights=losses_weights, optimizer="nadam", metrics={'layer6': "accuracy"})
#     print(similar_X_train)
    history = new_model.fit([X_train, similar_X_train], y={'layer6': y_train, tf_name: y_train, diff_name: y_train}, epochs=200,
    ...
    if args.saved:
        ...
        model_name = f'models/diff_german_{args.attr}_gated_{str(top_n)}_diff.h5'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--german_path', default='models/german_model.h5', help='model_path')
    ...
    german_train_scores = get_relevance(args.german_path, X_train,
                                        save_path=os.path.join('scores/german', os.path.basename(args.german_path) + ".score"))
    german_critical = get_critical_neurons(german_train_scores, args.percent)
    finals = []
    for top_n in [4]:
        protected_critical_ls = []
        for a in attrs:
            path = path_dict[a][top_n - 1]
            train_scores = get_relevance(path, X_train,  save_path=os.path.join('scores/german', os.path.basename(path) + ".score"))
            protected_critical = get_critical_neurons(train_scores, args.percent)
            protected_critical_ls.append(protected_critical)

        layer_num = len(german_critical)
        total_num = len(X_train)
        neurons = get_penalty_awarded(top_n, layer_num, total_num, german_critical, protected_critical_ls)
