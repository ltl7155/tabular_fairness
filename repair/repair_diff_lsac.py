...
def similar_set(X, num_attribs, protected_attribs, constraint, name):
    ...
    file_name = f"similar_set/lsac_{args.attr}_{name}_similar_set"
...
def get_penalty_awarded(top_n, layer_num, total_num, lsac_critical, protected_critical_ls):
    neurons = []

    for i in range(layer_num):
        lsac_layer_critical = lsac_critical[i].flatten()
        i_critical = my_filter(lsac_layer_critical, total_num)
...
def retrain(k, ps, neurons, para_res):
    ...
    new_model.load_weights(args.lsac_path, by_name=True)
    ...
    losses_weights = {'layer6': 4000.0, tf_name: 0.1, diff_name: 0.4}
...
#     print(similar_X_train)
    ...
    if args.saved:
        ...
        model_name = f'models/gated_models_diff/diff_lsac_{args.attr}_gated_{str(top_n)}_diff.h5'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lsac_path', default='models/lsac_model.h5', help='model_path')
    ...
    parser.add_argument('--l0', type=float, default=1000)
    ...
    lsac_train_scores = get_relevance(args.lsac_path, X_train,
                                        save_path=os.path.join('scores/lsac', os.path.basename(args.lsac_path) + ".score"))
    lsac_critical = get_critical_neurons(lsac_train_scores, args.percent)
    finals = []
    for top_n in [4]:
        ...
        layer_num = len(lsac_critical)
        total_num = len(X_train)
        neurons = get_penalty_awarded(top_n, layer_num, total_num, lsac_critical, protected_critical_ls)
