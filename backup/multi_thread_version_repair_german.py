...
def construct_model(neurons, top_layer, name, min, max, need_weights=True):
    ...
    d1 = ScaleLayer(50, min, max, 1, name='scalelayer1')
    layer2 = keras.layers.Dense(30, name="layer2")
    d2 = ScaleLayer(30, min, max, 2, name='scalelayer2')
    layer3 = keras.layers.Dense(15, name="layer3")
    d3 = ScaleLayer(15, min, max, 3, name='scalelayer3')
    layer4 = keras.layers.Dense(10, name="layer4")
    d4 = ScaleLayer(10, min, max, 4, name='scalelayer4')
    layer5 = keras.layers.Dense(5, name="layer5")
    d5 = ScaleLayer(5, min, max, 5, name='scalelayer5')
    layer6 = keras.layers.Dense(1, activation="sigmoid", name="layer6")
...
class myThread (threading.Thread):
    def __init__(self, threadID, name, neurons, para_res):
        threading.Thread.__init__(self)
        self.threadID = threadID
        self.name = name
        self.neurons = neurons
        self.para_res = para_res
    def run(self):
        print ("开启线程：" + self.name)
        retrain(self.neurons, self.para_res)
        print ("退出线程：" + self.name)

def retrain(neurons, para_res):
    keras.backend.clear_session()
    set_seed()
    while not exitFlag:
        queueLock.acquire()
        if not paras_queue.empty():
            k = paras_queue.qsize()
            ps = paras_queue.get()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--acc_lb', type=float, default=0.7)
    ...
    income_train_scores = get_relevance(args.income_path, X_train,
                                        save_path=os.path.join('scores/german/', os.path.basename(args.income_path) + ".score"))
    ...
    for top_n in [4]:
        ...
        if args.adjust_para:
            paras = [(a / 10, b / 10) for a in np.arange(-10, 0, 1) for b in np.arange(1, 10, 1)]
        else:
            paras = [(args.p0, args.p1)]

        paras_queue = queue.Queue()
        [paras_queue.put(p) for p in paras]
        para_res = dict()
        threads = []
        threadID = 1
        k = 0
        ...
        # create thread
        for t_n in range(thread_num):
            thread = myThread(threadID, "thread"+str(threadID), neurons, para_res)
            thread.start()
            threads.append(thread)
            threadID += 1
        while not paras_queue.empty():
            pass
    ...
    for k in para_res.keys():
        if para_res[k][0] > 0.7 and para_res[k][1] > 0.9:
            print(k, para_res[k])
