...
# model = keras.models.load_model('models/adult_race_model_1_0.99.h5')
# exit()
...
race_path_ls = ['models/adult_race_model_1_0.99.h5', 'models/adult_race_model_2_0.95.h5', "models/adult_race_model_3_0.88.h5", "models/adult_race_model_4_0.87.h5", "models/adult_race_model_5_0.86.h5"]
...
for top_n in [4]:
    ...
    race_path = race_path_ls[top_n - 1]
    race_train_scores = get_relevance(race_path, X_train, save_path=os.path.join('scores', os.path.basename(race_path) + ".score"))

    income_critical = get_critical_neurons(income_train_scores, percent)
    race_critical = get_critical_neurons(race_train_scores, percent)
    ...
    for i in range(layer_num):
        ...
        race_layer_critical = race_critical[i].flatten()
        ...
        g_unique, g_counts = np.unique(race_layer_critical, return_counts=True)
    ...
    for k, ps in enumerate(paras):
        ...
        if saved:
            ...
            model_name = f'models/repaired_models/race_gated_layer{str(top_n)}_per{str(percent)}_' \
                         f'thresh{str(weight_threshold)}_va{round(val_acc, 3)}_ra{round(repair_acc, 3)}.h5'
            saved_model = construct_model(results, top_n, name, ps[0], ps[1], need_weights=False)
