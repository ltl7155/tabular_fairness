...
def get_path_dict():
    saved_model_path = "models/finetuned_models_protected_attributes/compas/"
    path_ls = os.listdir(saved_model_path)
    path_dict = {}
    path_dict['r'] = [saved_model_path+p for p in path_ls if "r_compas" in p]
    path_dict['g'] = [saved_model_path+p for p in path_ls if "g_compas" in p]
    path_dict['r'].sort()
    path_dict['g'].sort()
    print(path_dict)
    return path_dict
...
def retrain(k, ps, neurons, para_res):
    ...
    if args.saved:
        ...
        model_name = f'models/compas_{args.attr}_gated_{str(top_n)}_{str(args.percent)}_{args.weight_threshold}_p{ps[0]}_p{ps[1]}.h5'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--income_path', default='models/compas_model.h5', help='model_path')
    parser.add_argument('--target_model_path', default='models/compas_EIDIG_INF_retrained_model.h5', help='model_path')
    ...
    X_train, X_val, y_train, y_val, constraint = pre_compas_scores.X_train, \
    pre_compas_scores.X_val, pre_compas_scores.y_train, pre_compas_scores.y_val, pre_compas_scores.constraint
    
    X_test, y_test = pre_compas_scores.X_test, pre_compas_scores.y_test
    target_model_path = args.target_model_path
    data_name = f"data/compas/Com-{args.attr}_ids_EIDIG_INF_1.npy"
    ...
    income_train_scores = get_relevance(args.income_path, X_train,
                                        save_path=os.path.join('scores/compas', os.path.basename(args.income_path) + ".score"))
    ...
    for top_n in [4]:
        ...
        for a in attrs:
            ...
            # path = "models/compas_race_model_4_0.87.h5"
            train_scores = get_relevance(path, X_train,  save_path=os.path.join('scores/compas', os.path.basename(path) + ".score"))
        ...
        for k in para_res.keys():
            ...
            file_path = f'records_compas_repair/{args.attr}_{args.percent}_{args.weight_threshold}/'
