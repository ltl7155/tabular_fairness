...
def get_acc(model,images,labels):
    ...
    print ("acc",acc,"y_hat.shape",y_hat.shape,"labels",labels.shape)
    print (np.unique(labels),"uniq.labels")
    print (np.unique(y_hat),"uniq.y_hat")
...
def main(args) -> None:
    ...
    name_prefix=name_prefix.replace("onlyweight_","")
