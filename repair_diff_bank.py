...
def get_path_dict():
    saved_model_path = "models/finetuned_models_protected_attributes/bank/"
    path_ls = os.listdir(saved_model_path)
    path_dict = {}
    path_dict['a'] = [saved_model_path+p for p in path_ls if "a_bank" in p]
    path_dict['a'].sort()
    print(path_dict)
    return path_dict
...
def similar_set(X, num_attribs, protected_attribs, constraint, name):
    ...
    file_name = f"similar_set/bank_{args.attr}_{name}_similar_set"
...
def get_penalty_awarded(top_n, layer_num, total_num, bank_critical, protected_critical_ls):
    neurons = []

    for i in range(layer_num):
        bank_layer_critical = bank_critical[i].flatten()
        i_critical = my_filter(bank_layer_critical, total_num)
...
def retrain(k, ps, neurons, para_res):
    ...
    new_model.load_weights(args.bank_path, by_name=True)
    ...
    losses_weights = {'layer6': 200.0, tf_name: 0.1, diff_name: 0.5}
...
if __name__ == '__main__':
    ...
    parser.add_argument('--bank_path', default='models/bank_model.h5', help='model_path')
    ...
    in_len = X_train.shape[1]
    data_name = f"data/bank/C-{args.attr}_ids_EIDIG_INF.npy"
    ...
    bank_train_scores = get_relevance(args.bank_path, X_train,
                                        save_path=os.path.join('scores/bank', os.path.basename(args.bank_path) + ".score"))
    bank_critical = get_critical_neurons(bank_train_scores, args.percent)
    finals = []
    for top_n in [4]:
        ...
        layer_num = len(bank_critical)
        total_num = len(X_train)
        neurons = get_penalty_awarded(top_n, layer_num, total_num, bank_critical, protected_critical_ls)
